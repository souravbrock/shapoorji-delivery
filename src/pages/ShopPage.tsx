  ...
  export default function ShopPage() {
    ...
    useEffect(() => {
      (async () => {
        try {
          // Fetch categories and products in parallel using Prisma
  ...
  focus:border-primary-400 focus:ring-2 focus:ring-primary-100 transition-all"
              />
              {search && (
                <button type="button" onClick={() => { setSearch(''); navigate('/shop'); }} className="absolute right-3
  ...
  ${!selectedCategory ? 'bg-primary-50 text-primary-700' : 'text-gray-600 hover:bg-gray-50'}`}
                >
                  All Products
                </button>
                {categories.map((cat) => (
                  <button
                    key={cat.id}
                    onClick={() => { setSelectedCategory(cat.slug); navigate(`/shop?category=${cat.slug}`);
  setShowFilters(false); }}
                    className={`w-full text-left px-3 py-2 rounded-lg text-sm font-medium transition-colors
  ${selectedCategory === cat.slug ? 'bg-primary-50 text-primary-700' : 'text-gray-600 hover:bg-gray-50'}`}
                  >
                    {cat.name}
                  </button>
                ))}
